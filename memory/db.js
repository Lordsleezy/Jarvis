...
async function initializeMemoryStore(basePath) {
  ...
  async function exportSmartContext(query) {
    ...
    let smart = '';
    if (q) {
      const hits = await findRelevantMemories(q, 5);
      if (hits.length > 0) {
        smart =
          'Most relevant stored memories for this prompt:\n' +
          hits
            .map((h) => `- [${h.category}] ${String(h.content).trim()}`)
            .join('\n') +
          '\n\n';
      }
    }
    return smart + exportFullContext();
  }
